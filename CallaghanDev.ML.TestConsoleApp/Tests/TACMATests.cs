...
{
    internal sealed class TACMATests : TestBase
    {
        ...
        {
            CountNumber++;
            TransformerTestSuite.PrintBanner($"{CountNumber} *  TACAMT (Time-Aware Cross-Attention with Memory & Temporal-decay)");
            ...
            {
                (Test_E2E_ContentAwareDecay_WithCompetition, "E2E: Content-aware decay prioritises recent high-impact news over older or low-impact news"),
                (Test_Decay_ContentAware_RecentSameContentStory_Beats_OlderSameContentStory_UnderCompetition, "Temporal: Recent same-content story beats older same-content story under competition"),
                (Test_Decay_ContentAware_HighImpactStory_HasLargerOutput_AtSameAge, "Semantic: Equally-old high-impact news produces larger output than equally-old low-impact news"),
                (Test_Decay_ContentAware_OlderHighImpact_Can_Outweigh_NewerLowImpact, "Semantic: Older high-impact news can outweigh newer low-impact news"),
                (Test_Decay_ContentAware_SameAgeDifferentContent_ChangesPrediction, "Semantic: Same-age different-content stories produce different predictions"),
                ...
                // ===========================================================
                (Test_PriceContext_ForwardWithContext_NoError, "PriceContext: ForwardWithPriceContextAndCache runs without error"),
                (Test_PriceContext_ForwardWithContext_OutputShape, "PriceContext: Output shape matches current chunk length"),
                (Test_PriceContext_ForwardWithContext_NoNaN, "PriceContext: No NaN in output"),
                (Test_PriceContext_EncodePriceHistory_NoCache, "PriceContext: EncodePriceHistory produces valid hidden states"),
                (Test_PriceContext_CombinedContext_NewsAndPrice, "PriceContext: Combined news + price context produces valid output"),

                // ===========================================================
                // PRICE CONTEXT TRAINING - Training Effectiveness
                // ===========================================================
                (Test_PriceContext_LossDecreases, "PriceContext: Loss decreases with price context training"),
                (Test_PriceContext_SingleSample_Overfit, "PriceContext: Single sample overfits with price context"),
                (Test_PriceContext_TrainingImproves_OverNoContext, "PriceContext: Training with context produces valid loss"),

                // ===========================================================
                // PRICE CONTEXT TRAINING - Behavioral Correctness
                // ===========================================================
                (Test_PriceContext_WithVsWithout_DifferentOutputs, "PriceContext: With vs without price context produces different outputs"),
                (Test_PriceContext_HistoryLength_AffectsOutput, "PriceContext: Different history lengths produce different outputs"),
                (Test_PriceContext_Deterministic, "PriceContext: Same split point produces same output"),
                (Test_PriceContext_SplitPoint_Respected, "PriceContext: Split respects min history and min current lengths"),

                // ===========================================================
                // PRICE CONTEXT TRAINING - Config Edge Cases
                // ===========================================================
                (Test_PriceContext_SequenceTooShort_SkipsSplit, "PriceContext: Sequence too short for split trains without context"),
                (Test_PriceContext_MinimalSplittableSequence, "PriceContext: Minimal splittable sequence works"),
                (Test_PriceContext_LargeHistorySmallCurrent, "PriceContext: Large history, small current chunk"),
                (Test_PriceContext_SmallHistoryLargeCurrent, "PriceContext: Small history, large current chunk"),

                // ===========================================================
                // SEQUENTIAL TRAINING
                // ===========================================================
                (Test_Sequential_MemoryAccumulates, "Sequential: Memory accumulates across samples"),
                (Test_Sequential_LossDecreases, "Sequential: Loss decreases over epoch"),
                (Test_Sequential_MemoryCleared_EachEpoch, "Sequential: Memory cleared at start of each epoch"),
                (Test_Sequential_PriceMemory_StoredFromForward, "Sequential: Price memory stored from forward pass hidden states"),

                // ===========================================================
                // NUMERICAL STABILITY
                // ===========================================================
                (Test_PriceContext_GradientClipping_NoNaN, "PriceContext: Gradient clipping prevents NaN with high LR"),
                (Test_PriceContext_NoNaN_AfterManyEpochs, "PriceContext: No NaN after extended training"),

                // ===========================================================
                // TACAMT CORE - Multi-Story Forward
                // ===========================================================
                (Test_MultiStory_ForwardNoError, "TACAMT: Multi-Story Forward No Error"),
                (Test_MultiStory_ForwardOutputShape, "TACAMT: Multi-Story Forward Output Shape"),
                (Test_MultiStory_DifferentArrivalTimes_DifferentOutputs, "TACAMT: Different Arrival Times => Different Outputs"),

                // ===========================================================
                // TACAMT CORE - Time Decay
                // ===========================================================
                (Test_TimeDecay_InitialValues, "TACAMT: Time-Decay Initial Values"),
                (Test_TimeDecay_ParamsChangeAfterTraining, "TACAMT: Decay Params Change After Training"),

                // ===========================================================
                // TACAMT CORE - Training
                // ===========================================================
                (Test_TACAMT_MultiStory_LossDecreases, "TACAMT: Multi-Story Loss Decreases"),
                (Test_TACAMT_MultiStory_SingleSampleOverfit, "TACAMT: Multi-Story Single Sample Overfit"),

                // ===========================================================
                // TACAMT CORE - Backward Compatibility
                // ===========================================================
              
                (Test_BackwardCompat_NullText_NoError, "TACAMT: Backward-Compat Null Text"),
                (Test_BackwardCompat_TrainWithIntArrays, "TACAMT: Backward-Compat Train int[][]"),

                // ===========================================================
                // TACAMT CORE - Edge Cases
                // ===========================================================
                (Test_MixedBatch_SomeStoriesSomeNull, "TACAMT: Mixed Batch Stories+Null"),
                (Test_NegativeArrivalTime_NoError, "TACAMT: Negative Arrival Times"),
                (Test_ManyStories_NoError, "TACAMT: Many Stories (12) No Error"),
                (Test_SingleTokenStory_NoError, "TACAMT: Single Token Story"),

                // ===========================================================
                // TACAMT CORE - Memory Management
                // ===========================================================
                (Test_UpdateNewsMemory_StoresEntries,
                    "TACAMT: UpdateNewsMemory Stores Entries"),
                (Test_ClearNewsMemory_Resets,
                    "TACAMT: ClearNewsMemory Resets"),
                (Test_UpdateNewsMemory_TrimOldEntries,
                    "TACAMT: UpdateNewsMemory Trims Old"),
                (Test_PredictWithMemory_NoMemory_FallsBack,
                    "TACAMT: PredictWithMemory No Memory Fallback"),
                (Test_PredictWithMemory_WithMemory_NoError,
                    "TACAMT: PredictWithMemory With Memory"),
                (Test_PredictWithMemory_OldStoriesDecayed,
                    "TACAMT: PredictWithMemory Old Stories Decayed"),

                // ===========================================================
                // TACAMT CORE - Save/Load
                // ===========================================================
                (Test_SaveLoad_DecayParams_Preserved,
                    "TACAMT: SaveLoad Decay Params Preserved"),
                (Test_SaveLoad_NewsMemory_Preserved,
                    "TACAMT: SaveLoad News Memory Preserved"),
                (Test_SaveLoad_ForwardMatch,
                    "TACAMT: SaveLoad Forward Match"),
                (Test_SaveLoad_ContinueTraining,
                    "TACAMT: SaveLoad Continue Training"),

                // ===========================================================
                // TACAMT CORE - Determinism & Stability
                // ===========================================================
                (Test_Deterministic_MultiStory,
                    "TACAMT: Deterministic Multi-Story Forward"),
                (Test_GradientClipping_WithDecay_NoNaN,
                    "TACAMT: Gradient Clipping With Decay No NaN"),
                (Test_Validate_MultiStory,
                    "TACAMT: Validate Multi-Story Computable"),
                (Test_PerHeadDecayRates_Diverge,
                    "TACAMT: Per-Head Decay Rates Diverge"),

                // ===========================================================
                // TACAMT CORE - Price-Only (No Text)
                // ===========================================================
                (Test_PriceOnly_ForwardNoError, "TACAMT: Price-Only Forward"),
                (Test_PriceOnly_LossDecreases, "TACAMT: Price-Only Loss Decreases"),
                (Test_FrozenTextEncoder_DecayStillLearns, "TACAMT: Frozen Text Encoder Decay Still Learns"),

                // ===========================================================
                // ADDITIONAL - Config Validation
                // ===========================================================
                (Test_Config_Validate_ThrowsOnBadDims, "Config: Validate throws on bad dimensions"),
                (Test_Config_Validate_AcceptsGoodConfig, "Config: Validate accepts good config"),

                // ===========================================================
                // ADDITIONAL - Memory Pruning
                // ===========================================================
                (Test_MemoryPruning_AttentionBased_KeepsHighScoreEntries, "MemoryPruning: Attention-based keeps high-score entries"),
                (Test_MemoryPruning_OldestFirst_WhenDisabled, "MemoryPruning: Oldest-first when attention pruning disabled"),
                (Test_MemoryPruning_ColdStartProtection, "MemoryPruning: Cold-start protection for new entries"),

                // ===========================================================
                // ADDITIONAL - ContentAwareDecayNetwork Unit Tests
                // ===========================================================
                (Test_DecayNetwork_ForwardProduces_ValidBias, "DecayNetwork: Forward produces valid bias"),
                (Test_DecayNetwork_BackwardProduces_NonZeroGrads, "DecayNetwork: Backward produces non-zero gradients"),
                (Test_DecayNetwork_DropoutChangesOutput, "DecayNetwork: Dropout changes output (training vs inference)"),
                (Test_DecayNetwork_WeightDecay_AddedToGrads, "DecayNetwork: Weight decay added to gradients"),
                (Test_DecayNetwork_SaveLoad_Roundtrip, "DecayNetwork: Save/Load roundtrip preserves weights"),

                // ===========================================================
                // ADDITIONAL - Confidence Head
                // ===========================================================
                (Test_ConfidenceHead_Outputs_SigmoidRange, "ConfidenceHead: Outputs in [0, 1] range"),
                (Test_ConfidenceHead_TrainingUpdatesWeights, "ConfidenceHead: Training updates confidence weights"),

                // ===========================================================
                // ADDITIONAL - Tokenizer Integration
                // ===========================================================
                (Test_Tokenizer_TokenizeStories_WithoutTokenizer_Throws, "Tokenizer: TokenizeStories without tokenizer throws"),
                ...
                // ===========================================================
                (Test_Decay_RecentNews_HigherAttention_ThanOld, "Semantic: Recent news receives higher attention weight than old news"),
                (Test_ConfidenceHead_CorrelatesWithAccuracy, "Semantic: Confidence higher on predictable vs noisy samples"),
                (Test_PriceContext_LongerHistory_LowerLoss, "Semantic: Longer price history context reduces validation loss"),
                (Test_Sequential_MemoryChangesOutput_VsNoMemory_UsingPredictWithMemory, "Semantic: Prediction with memory differs from prediction without"),
                (Test_MultiStory_MoreContext_ChangesOutput, "Semantic: Adding more news stories changes the prediction"),
                (Test_Decay_OlderMemory_LessInfluence, "Semantic: Memory entries at t=-1000 have less influence than t=-1"),
            };
        ...
        public void Test_PriceContext_HistoryGrads_DoNotFlowBack()
        {
            ...
            {
                priceCtxTimes[t] = -(splitPoint - t);
            }

            var cache = new MultimodalForwardCache(config.Text.NumLayers, config.Price.NumLayers);
            var (pred, conf) = model.ForwardWithPriceContextAndCache(stories[0], currentInput, priceCtxHidden, priceCtxTimes, cache);
            ...
            var (pred2, _) = model.ForwardWithPriceContextAndCache(stories[0], currentInput, perturbedCtx, priceCtxTimes, cache2);
            ...
            for (int i = 0; i < pred.GetLength(0) && !outputsDiffer; i++)
            {
                ...
                {
                    if (MathF.Abs(pred[i, j] - pred2[i, j]) > 1e-6f)
                    {
                        outputsDiffer = true;
                    }
                }
            ...
            Assert(cache.PriceBlockCaches.Count == config.Price.NumLayers, "Cache should only have block caches for the current chunk");

            int currentSeqLen = currentInput.GetLength(0);
            var firstBlockCache = cache.PriceBlockCaches[0];

            Assert(firstBlockCache.BlockInput.GetLength(0) == currentSeqLen, $"Block cache input rows should be {currentSeqLen}, got {firstBlockCache.BlockInput.GetLength(0)}");
        }
        ...
        public void Test_PriceContext_HistoryEncoder_WeightsUnchanged_SingleStep()
        {
            ...
            for (int i = 0; i < histHidden1.GetLength(0) && !encodingsDiffer; i++)
            {
                ...
                {
                    if (MathF.Abs(histHidden1[i, j] - histHidden2[i, j]) > 1e-6f)
                    {
            ...
            Assert(encodingsDiffer, "History encoding should change after training (shared weights updated via current chunk)");
        }
        ...
        public void Test_PriceContext_DecayNetwork_GetsGradsFromPriceContext()
        {
            ...
            Assert(VectorChanged(logDecayBefore, decayNet.LogBaseDecayRate), "LogBaseDecayRate should change");

            Assert(!TensorAllClose(w1Before, decayNet.W1, 1e-7f), "Decay W1 should change - gradients should flow");
        }
        ...
        public void Test_PriceContext_CrossAttnKV_GradsOnlyForNews_NotPriceCtx()
        {
            ...
            Assert(MatrixChanged(textEmbBefore, model.TextTokenEmbedding), "Text embedding should change - news context K/V gradients flow back to text encoder");
        }
        ...
        public void Test_PriceContext_ForwardWithContext_NoError()
        {
            ...
            var (pred, _) = model.ForwardWithPriceContextAndCache(stories[0], SliceRows(priceInputs[0], 8, 15), priceCtxHidden, priceCtxTimes, cache);
        ...
        public void Test_PriceContext_ForwardWithContext_OutputShape()
        {
            ...
            var (pred, _) = model.ForwardWithPriceContextAndCache(stories[0], currentInput, priceCtxHidden, priceCtxTimes, cache);
        ...
        public void Test_PriceContext_CombinedContext_NewsAndPrice()
        {
            ...
            var cache = new MultimodalForwardCache(config.Text.NumLayers, config.Price.NumLayers);
            var (pred, _) = model.ForwardWithPriceContextAndCache(stories[0], SliceRows(priceInputs[0], 6, 15), priceCtxHidden, priceCtxTimes, cache);
        ...
        public void Test_PriceContext_WithVsWithout_DifferentOutputs()
        {
            ...
            var priceCtxTimes = Enumerable.Range(0, 8).Select(t => -(float)(8 - t)).ToArray();
            var cache = new MultimodalForwardCache(config.Text.NumLayers, config.Price.NumLayers);
            var (predWith, _) = model.ForwardWithPriceContextAndCache(stories[0], currentInput, priceCtxHidden, priceCtxTimes, cache);
            ...
            for (int i = 0; i < predNo.GetLength(0) && !anyDiff; i++)
            {
                ...
                {
                    if (MathF.Abs(predNo[i, j] - predWith[i, j]) > 1e-6f)
                    {
                        anyDiff = true;
                    }
                }
        ...
        public void Test_PriceContext_SequenceTooShort_SkipsSplit()
        {
            ...
            for (int s = 0; s < numSamples; s++)
            {
                ...
                for (int t = 0; t < 8; t++)
                {
                    for (int f = 0; f < 5; f++)
                    {
                        priceInputs[s][t, f] = (float)rng.NextDouble();
                        priceTargets[s][t, f] = (float)rng.NextDouble();
                    }
                }
            }
        ...
        public void Test_PriceContext_MinimalSplittableSequence()
        {
            ...
            var priceInputs = new float[1][,]
            { 
                new float[11, 5] 
            };

            var priceTargets = new float[1][,]
            {
                new float[11, 5] 
            };
            ...
            for (int t = 0; t < 11; t++)
            {
                for (int f = 0; f < 5; f++)
                {
                    priceInputs[0][t, f] = (float)rng.NextDouble();
                    priceTargets[0][t, f] = (float)rng.NextDouble();
                }
            }
        ...
        public void Test_PriceContext_NoNaN_AfterManyEpochs()
        {
            ...
            for (int layer = 0; layer < config.Price.NumLayers; layer++)
            {
                ...
                {
                    Assert(!float.IsNaN(decayNet.LogBaseDecayRate[h]), $"Layer {layer} head {h} LogBaseDecayRate is NaN");
                }
            }
        }
        ...
        public void Test_MultiStory_ForwardNoError()
        {
        ...
        public void Test_MultiStory_ForwardOutputShape()
        {
        ...
        public void Test_MultiStory_DifferentArrivalTimes_DifferentOutputs()
        {
        ...
        #region Forward - varying sequence lengths
        public void Test_Forward_SeqLen1_Works() 
        { 
            var m = new TACAMT_Model(CreateConfig(priceSeqLen: 5), new Random(42)); 
            var p = new float[1, 5]; 
            for (int j = 0; j < 5; j++)
            {
                p[0, j] = 0.5f;
            }
            var (pred, _) = m.Forward((NewsStory[])null, p); 
            
            Assert(pred.GetLength(0) == 1 && !HasNaN(pred), "Failed"); 
        }
        public void Test_Forward_SeqLen2_Works() 
        { 
            var m = new TACAMT_Model(CreateConfig(priceSeqLen: 5), new Random(42)); 
            var rng = new Random(42); 
            var p = new float[2, 5]; 
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < 5; j++)
                {
                    p[i, j] = (float)rng.NextDouble();
                }
            }

            var (pred, _) = m.Forward((NewsStory[])null, p);

            Assert(pred.GetLength(0) == 2 && !HasNaN(pred), "Failed");
        }
        public void Test_Forward_MaxSeqLen_Works() 
        { 
            var m = new TACAMT_Model(CreateConfig(priceSeqLen: 20), new Random(42));
            var p = RandomMatrix(20, 5, new Random(42), 0.5f);
            var (pred, _) = m.Forward((NewsStory[])null, p);

            Assert(pred.GetLength(0) == 20 && !HasNaN(pred), "Failed"); 
        }
        public void Test_Forward_DifferentFeatureDim_Works() 
        { 
            var m = new TACAMT_Model(CreateConfig(inputFeatures: 3, priceSeqLen: 10), new Random(42)); 
            var (pred, _) = m.Forward((NewsStory[])null, RandomMatrix(8, 3, new Random(42), 0.5f)); 
            Assert(pred != null && !HasNaN(pred), "Failed"); 
        }
        public void Test_Forward_DifferentOutputDim_Works() 
        { 
            var m = new TACAMT_Model(CreateConfig(outputDim: 1, priceSeqLen: 10), new Random(42)); 
            var (pred, _) = m.Forward((NewsStory[])null, RandomMatrix(8, 5, new Random(42), 0.5f));
            Assert(pred.GetLength(1) == 1, "Output dim should be 1"); 
        }
        #endregion

        #region Forward - Context routing
        public void Test_Forward_NullStories_NullText_NoContext()
        { 
            var m = new TACAMT_Model(CreateConfig(), new Random(42)); var (pred, _) = m.Forward((NewsStory[])null, RandomMatrix(10, 5, new Random(42), 0.5f));

            Assert(pred != null && !HasNaN(pred), "Failed"); 
        }
        public void Test_Forward_EmptyStories_NoContext() 
        { 
            var m = new TACAMT_Model(CreateConfig(), new Random(42)); var (pred, _) = m.Forward(new NewsStory[0], RandomMatrix(10, 5, new Random(42), 0.5f)); 
            Assert(pred != null && !HasNaN(pred), "Failed");
        }
        public void Test_Forward_SingleStory_CrossAttnActivated() 
        { 
            var (tok, _, pi, _) = CreateTestData(1);
            var c = CreateConfig(tok.VocabSize + 2);
            var m = new TACAMT_Model(c, new Random(42));

            var tokens = tok.Encode("stock price rose", addSpecialTokens: true); 
            var no = m.Forward((NewsStory[])null, pi[0]); 
            var wi = m.Forward(new[] { new NewsStory(tokens, 0f) }, pi[0]);

            bool d = false;

            for (int i = 0; i < no.predictions.GetLength(0) && !d; i++)
            {
                for (int j = 0; j < no.predictions.GetLength(1) && !d; j++)
                {
                    if (MathF.Abs(no.predictions[i, j] - wi.predictions[i, j]) > 1e-6f) d = true;
                }
            }
            
            Assert(d, "Story should change output");
        }
        public void Test_Forward_MultipleStories_AllContribute() 
        { 
            var (tok, _, pi, _) = CreateTestData(1);
            var c = CreateConfig(tok.VocabSize + 2);
            var m = new TACAMT_Model(c, new Random(42));
            var t1 = tok.Encode("stock price rose", addSpecialTokens: true);
            var t2 = tok.Encode("market crashed", addSpecialTokens: true); 
            var (p1, _) = m.Forward(new[] { new NewsStory(t1, 0f) }, pi[0]);
            var (p2, _) = m.Forward(new[] { new NewsStory(t1, 0f), new NewsStory(t2, 1f) }, pi[0]); 
            bool d = false; 
            for (int i = 0; i < p1.GetLength(0) && !d; i++)
            {
                for (int j = 0; j < p1.GetLength(1) && !d; j++)
                {
                    if (MathF.Abs(p1[i, j] - p2[i, j]) > 1e-6f) d = true;
                }
            }
            Assert(d, "Second story should change output"); }
        public void Test_Forward_ContextHidden_DimMatchesPriceEmb()
        { 
            var (tok, stories, pi, _) = CreateTestData(1);

            var c = CreateConfig(tok.VocabSize + 2, embDim: 16); 
            var m = new TACAMT_Model(c, new Random(42));
            var cache = new MultimodalForwardCache(c.Text.NumLayers, c.Price.NumLayers);

            m.ForwardWithCache(stories[0], pi[0], cache);
        ...
        public void Test_MultiLayer_MoreLayers_DifferentOutput() 
        {
            ...
            var m1 = new TACAMT_Model(CreateConfig(tok.VocabSize + 2, numLayers: 1), new Random(42)); 
            var m2 = new TACAMT_Model(CreateConfig(tok.VocabSize + 2, numLayers: 2), new Random(42));

            var (p1, _) = m1.Forward(stories[0], pi[0]); 
            var (p2, _) = m2.Forward(stories[0], pi[0]);
            bool d = false; 
            ...
            Assert(d, "Different layers should differ"); 
        }
        public void Test_MultiLayer_AllBlocks_Executed() 
        { 
            var (tok, stories, pi, _) = CreateTestData(1);
            var c = CreateConfig(tok.VocabSize + 2, numLayers: 3);
            var m = new TACAMT_Model(c, new Random(42)); 
            var cache = new MultimodalForwardCache(c.Text.NumLayers, c.Price.NumLayers); 
            m.ForwardWithCache(stories[0], pi[0], cache); 
            Assert(cache.PriceBlockCaches.Count == 3, $"Got {cache.PriceBlockCaches.Count}"); 
            
            for (int i = 0; i < 3; i++)
            {
                Assert(cache.PriceBlockCaches[i].BlockInput != null, $"Block {i} null");
            }
        }
        public void Test_MultiLayer_DeepModel_NoNaN() 
        { 
            var (tok, stories, pi, _) = CreateTestData(1); 
            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2, numLayers: 4, embDim: 16, numHeads: 2), new Random(42));
            var (pred, _) = m.Forward(stories[0], pi[0]); 

            Assert(!HasNaN(pred), "4-layer NaN");
        }
        ...
        {
            var m = new TACAMT_Model(CreateConfig(), new Random(42)); 
            var rng = new Random(42); 
            var p1 = new float[10, 5]; 
            var p2 = new float[10, 5];
            ...
            var (r1, _) = m.Forward((NewsStory[])null, p1); 
            var (r2, _) = m.Forward((NewsStory[])null, p2); 
            bool d = false;
            ...
            for (int i = 0; i < 10 && !d; i++)
            {
                for (int j = 0; j < 5 && !d; j++)
                {
                    if (MathF.Abs(r1[i, j] - r2[i, j]) > 1e-6f)
                    {
                        d = true;
                    }
                }
            ...
            Assert(d, "Reversed should differ"); }

        #endregion

        #region Causal masking

        public void Test_CausalMask_FutureTokens_DontAffectPast() 
        { 
            var m = new TACAMT_Model(CreateConfig(priceSeqLen: 12), new Random(42)); 
            var p1 = RandomMatrix(10, 5, new Random(42), 0.5f); 
            var p2 = (float[,])p1.Clone();
            ...
            for (int i = 0; i < 7; i++)
            {
                for (int j = 0; j < 5; j++)
                {
                    Assert(MathF.Abs(r1[i, j] - r2[i, j]) < 1e-5f, $"Pos {i} affected by future");
                }
            }
        }
        public void Test_CausalMask_Price_DecoderOnly_Enabled() 
        { 
            Assert(CreateConfig().Price.UseDecoderOnly == true, "Should be true");
        }

        #endregion

        #region Gradient numerical checks
        ...
        public void Test_GradCheck_OutputProjection_ApproximatelyCorrect()
        {
            ...
            var m = new TACAMT_Model(c, new Random(42)); var rng = new Random(42);
            ...
            var grads = new Gradients(c);
            int sl = pred.GetLength(0), od = c.Output.OutputDim, ed = c.Price.EmbeddingDim;
            ...
            for (int t = 0; t < sl; t++)
            {
                for (int v = 0; v < od; v++)
                {
                    for (int e = 0; e < ed; e++)
                    {
                        grads.OutputProjectionGrad[v, e] += cache.PriceFinalHidden[t, e] * 2f * (pred[t, v] - target[t, v]) / (sl * od);
                    }
                }
            }
            float eps = 1e-3f; float origW = m.OutputProjection[0, 0];
            m.OutputProjection[0, 0] = origW + eps; var (pp, _) = m.Forward((NewsStory[])null, SliceRows(price, 0, 5)); 
            float lp = 0; 
            ...
            lp /= (sl * od);
            m.OutputProjection[0, 0] = origW - eps; var (pm, _) = m.Forward((NewsStory[])null, SliceRows(price, 0, 5)); 
            float lm = 0; 
            ...
            lm /= (sl * od);
            m.OutputProjection[0, 0] = origW;

            float fd = (lp - lm) / (2 * eps);
            float ag = grads.OutputProjectionGrad[0, 0];
            float re = MathF.Abs(fd - ag) / (MathF.Abs(fd) + MathF.Abs(ag) + 1e-8f);
            
            Assert(re < 0.1f, $"analytical={ag:E4} fd={fd:E4} relErr={re:F4}");
        }
        public void Test_GradCheck_PriceInputProjection_ApproximatelyCorrect()
        {
            ...
            var price = RandomMatrix(6, 5, rng, 0.3f); 
            var target = RandomMatrix(5, 3, rng, 0.3f); 
            float eps = 1e-3f; 
            int sl = 5, od = 3;

            float ow = m.PriceInputProjection[0, 0];

            m.PriceInputProjection[0, 0] = ow + eps; var (pp, _) = m.Forward((NewsStory[])null, SliceRows(price, 0, 5)); float lp = 0; for (int t = 0; t < sl; t++) for (int j = 0; j < od; j++) { float d = pp[t, j] - target[t, j]; lp += d * d; }
        ...
        #region Training parameter updates

        public void Test_Train_AllPriceBlockParams_Updated() 
        { 
            var (tok, st, pi, pt) = CreateTestData(5); 
            var c = CreateConfig(tok.VocabSize + 2); 
            var m = new TACAMT_Model(c, new Random(42)); 

            var b = m.PriceBlocks[0];

            var swk = CloneMatrix(b.SelfAttention.WK);
            var cwk = CloneMatrix(b.CrossAttention.WK); 
            var lng = CloneVector(b.LnCrossGamma); 
            var lff = CloneVector(b.LNFFNGamma);

            new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.01f, BatchSize = 5, Epochs = 5, Verbose = false }).Train(st, pi, pt);

            Assert(MatrixChanged(swk, b.SelfAttention.WK), "SelfWK");
            Assert(MatrixChanged(cwk, b.CrossAttention.WK), "CrossWK");
            Assert(VectorChanged(lng, b.LnCrossGamma), "LNCross"); 
            Assert(VectorChanged(lff, b.LNFFNGamma), "LNFFN"); 
        }
        public void Test_Train_OutputBias_Updated()
        { 
            var (tok, st, pi, pt) = CreateTestData(5);

            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2), new Random(42));

            var bb = CloneVector(m.OutputBias);

            new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.01f, BatchSize = 5, Epochs = 5, Verbose = false }).Train(st, pi, pt); 

            Assert(VectorChanged(bb, m.OutputBias), "Should change");
        }
        public void Test_Train_PriceInputProjectionBias_Updated() 
        { 
            var (tok, st, pi, pt) = CreateTestData(5);
            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2), new Random(42));

            var bb = CloneVector(m.PriceInputProjectionBias);

            new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.01f, BatchSize = 5, Epochs = 5, Verbose = false }).Train(st, pi, pt);

            Assert(VectorChanged(bb, m.PriceInputProjectionBias), "Should change"); 
        }
        public void Test_Train_TextEncoder_Updated_WhenNotFrozen() 
        { 
            var (tok, st, pi, pt) = CreateTestData(5);
            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2, freezeText: false), new Random(42)); 
            var eb = CloneMatrix(m.TextTokenEmbedding); 
            new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.01f, BatchSize = 5, Epochs = 5, Verbose = false }).Train(st, pi, pt);
            Assert(MatrixChanged(eb, m.TextTokenEmbedding), "Should change");
        }

        public void Test_Train_TextEncoder_NOT_Updated_WhenFrozen() 
        { 
            var (tok, st, pi, pt) = CreateTestData(5);
            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2, freezeText: true), new Random(42));

            var eb = CloneMatrix(m.TextTokenEmbedding);

            new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.01f, BatchSize = 5, Epochs = 5, Verbose = false }).Train(st, pi, pt);

            Assert(!MatrixChanged(eb, m.TextTokenEmbedding), "Should NOT change");
        }
        public void Test_Train_FFN_Weights_Updated() 
        { 
            var (tok, st, pi, pt) = CreateTestData(5);
            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2), new Random(42)); var (pb, _) = m.Forward(st[0], pi[0]);
            new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.01f, BatchSize = 5, Epochs = 10, Verbose = false }).Train(st, pi, pt);

            var (pa, _) = m.Forward(st[0], pi[0]); Assert(MatrixChanged(pb, pa), "FFN changed (output differs)");
        }

        public void Test_Train_AllDecayNetworkParams_Updated() 
        { 
            var (tok, st, pi, pt) = CreateTestData(5);
            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2), new Random(42));
            var dn = m.PriceBlocks[0].DecayNetwork; var lb = CloneVector(dn.LogBaseDecayRate);

            var b2b = CloneVector(dn.B2);
            var w2b = CloneMatrix(dn.W2);
            var tlf = CloneMatrix(dn.TimeLogFreq);

            new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.01f, BatchSize = 5, Epochs = 10, Verbose = false }).Train(st, pi, pt);

            Assert(VectorChanged(lb, dn.LogBaseDecayRate), "LogBase"); 
            Assert(VectorChanged(b2b, dn.B2), "B2");
            Assert(MatrixChanged(w2b, dn.W2), "W2");
            Assert(MatrixChanged(tlf, dn.TimeLogFreq), "TimeLogFreq");
        }

        #endregion

        #region Training Loss behavior

        public void Test_Train_ZeroTarget_LossDecreases() 
        { 
            var c = CreateConfig(priceSeqLen: 12); 
            var m = new TACAMT_Model(c, new Random(42)); 
            var rng = new Random(42);
            int n = 3; 
            var pi = new float[n][,];
            var pt = new float[n][,];
            var ns = new NewsStory[n][];

            for (int s = 0; s < n; s++)
            { 
                pi[s] = RandomMatrix(10, 5, rng, 0.5f);
                pt[s] = new float[10, 5];
            }

            var t1 = new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.001f, BatchSize = 3, Epochs = 1, Verbose = false });
            float lb = t1.Validate(ns, pi, pt);
            new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.005f, BatchSize = 3, Epochs = 50, Verbose = false }).Train(ns, pi, pt);

            float la = t1.Validate(ns, pi, pt);
            Assert(la < lb, $"{lb:F6} -> {la:F6}");
        }
        ...
        { 
            ...
            var m1 = new TACAMT_Model(c, new Random(42));
            var t1 = new TACAMT_Trainer(m1, new TrainingConfig { LearningRate = 0.001f, BatchSize = 1, Epochs = 5, Verbose = false }); 

            float lb1 = t1.Validate(st, pi, pt); 
            t1.Train(st, pi, pt); 

            float la1 = t1.Validate(st, pi, pt); 

            var m5 = new TACAMT_Model(c, new Random(42)); 
            var t5 = new TACAMT_Trainer(m5, new TrainingConfig { LearningRate = 0.001f, BatchSize = 5, Epochs = 5, Verbose = false });

            float lb5 = t5.Validate(st, pi, pt); 
            t5.Train(st, pi, pt);

            float la5 = t5.Validate(st, pi, pt); 
            Assert(la1 < lb1, "BS1"); 
            Assert(la5 < lb5, "BS5");
        }
        ...
        { 
            ...
            new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.01f, BatchSize = 5, Epochs = 10, UseLearningRateDecay = true, LearningRateDecay = 0.95f, Verbose = false }).Train(st, pi, pt);
            
            float l = new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.01f, BatchSize = 5, Epochs = 1, Verbose = false }).Validate(st, pi, pt);
            Assert(!float.IsNaN(l) && l >= 0, $"Invalid loss: {l}");
        }
        public void Test_Train_HighLR_WithClipping_NoExplosion() 
        { 
            var (tok, st, pi, pt) = CreateTestData(5); 
            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2), new Random(42)); 
            new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.5f, BatchSize = 5, Epochs = 5, UseGradientClipping = true, GradientClipThreshold = 0.5f, Verbose = false }).Train(st, pi, pt);
            
            var (pred, _) = m.Forward(st[0], pi[0]); 
            Assert(!HasNaN(pred), "NaN after high LR");
        }

        #endregion

        #region Gradient clipping
        public void Test_GradClip_NormReduced() 
        { 
            var (tok, st, pi, pt) = CreateTestData(3);
            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2), new Random(42)); new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 1.0f, BatchSize = 3, Epochs = 3, UseGradientClipping = true, GradientClipThreshold = 0.1f, Verbose = false }).Train(st, pi, pt);
            float l = new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.001f, BatchSize = 3, Epochs = 1, Verbose = false }).Validate(st, pi, pt);

            Assert(!float.IsNaN(l) && !float.IsInfinity(l), "Should prevent NaN");
        }
        #endregion

        #region Predict next
        public void Test_PredictNext_ReturnsLastTimestep() 
        { 
            var m = new TACAMT_Model(CreateConfig(), new Random(42)); var (pred, _) = m.PredictNext((NewsStory[])null, RandomMatrix(10, 5, new Random(42), 0.5f)); 
            Assert(pred.Length == 5 && !HasNaN(pred), "Failed");
        }
        public void Test_PredictNext_ConfidenceInRange() 
        { 
            var m = new TACAMT_Model(CreateConfig(useConfidence: true), new Random(42));

            var (_, conf) = m.PredictNext((NewsStory[])null, RandomMatrix(10, 5, new Random(42), 0.5f));

            Assert(conf >= 0f && conf <= 1f, $"Conf {conf}");
        }
        public void Test_PredictNext_DefaultConfidence1_WhenDisabled()
        { 
            var m = new TACAMT_Model(CreateConfig(useConfidence: false), new Random(42)); var (_, conf) = m.PredictNext((NewsStory[])null, RandomMatrix(10, 5, new Random(42), 0.5f)); 
            Assert(conf == 1.0f, $"Got {conf}");
        }

        // === PREDICT WITH MEMORY ===
        public void Test_PredictWithMemory_AccumulatesNewsAndPrice()
        { 
            var (tok, st, pi, _) = CreateTestData(1);
            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2), new Random(42)); m.PredictWithMemory(st[0], pi[0], 100.0);

            Assert(m.NewsMemory.Count > 0, "News"); Assert(m.PriceMemory.Count > 0, "Price"); 
        }
        public void Test_PredictWithMemory_PrunesWhenOverLimit() 
        { 
            var (tok, st, pi, _) = CreateTestData(5);
            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2), new Random(42));

            for (int i = 0; i < 5; i++)
            {
                m.PredictWithMemory(st[i], pi[i], (i + 1) * 100.0, maxNewsMemorySize: 5, maxPriceMemorySize: 10);
            }

            Assert(m.NewsMemory.Count <= 5, $"News {m.NewsMemory.Count}"); Assert(m.PriceMemory.Count <= 10, $"Price {m.PriceMemory.Count}");
        }
        public void Test_PredictWithMemory_MultipleCallsChain()
        { 
            var (tok, st, pi, _) = CreateTestData(3);
            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2), new Random(42)); m.PredictWithMemory(st[0], pi[0], 100.0);

            int n1 = m.NewsMemory.Count; m.PredictWithMemory(st[1], pi[1], 200.0);
            Assert(m.NewsMemory.Count > n1, "Should accumulate");
        }
        public void Test_PredictWithMemory_TimestampOrdering_Preserved() 
        {
            var (tok, st, pi, _) = CreateTestData(2);
            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2), new Random(42));

            m.PredictWithMemory(st[0], pi[0], 100.0);

            m.PredictWithMemory(st[1], pi[1], 200.0);

            Assert(m.NewsMemory.Max(e => e.AbsoluteTimestamp) >= 200.0, "Latest ts");
        }

        #endregion

        #region Memory pruning 
        ...
        { 
            ...
            for (int i = 0; i < 10; i++)
            {
                m.PriceMemory.Add(new PriceMemoryEntry { HiddenState = new float[c.Price.EmbeddingDim], AbsoluteTimestamp = i * 10.0, AttentionScore = i * 0.1f, QueryCount = 5 });
            }
            typeof(TACAMT_Model).GetMethod("PricePruneMemory", BindingFlags.Instance | BindingFlags.NonPublic).Invoke(m, new object[] { 5 });

            Assert(m.PriceMemory.Count == 5, $"Got {m.PriceMemory.Count}");
        }
        ...
        { 
            ...
            m.PruningConfig.UseAttentionBasedPruning = true;
            m.PruningConfig.NewEntryReserveFraction = 0.4f;
            m.PruningConfig.MinQueryCountForPruning = 1;

            for (int i = 0; i < 20; i++)
            {
                m.NewsMemory.Add(new NewsMemoryEntry { HiddenState = new float[c.Price.EmbeddingDim], AbsoluteTimestamp = i * 10.0, AttentionScore = 0.01f, QueryCount = 10 });
            }

            typeof(TACAMT_Model).GetMethod("PruneNewsMemory", BindingFlags.Instance | BindingFlags.NonPublic).Invoke(m, new object[] { 10 }); Assert(m.NewsMemory.Max(e => e.AbsoluteTimestamp) >= 190.0, "Newest should survive");
        }
        public void Test_Pruning_ZeroMaxSize_ClearsAll() 
        { 
            var c = CreateConfig(); 
            var m = new TACAMT_Model(c, new Random(42)); 
            ...
            for (int i = 0; i < 5; i++)
            {
                m.NewsMemory.Add(new NewsMemoryEntry { HiddenState = new float[c.Price.EmbeddingDim], AbsoluteTimestamp = i });
            }

            typeof(TACAMT_Model).GetMethod("PruneNewsMemory", BindingFlags.Instance | BindingFlags.NonPublic).Invoke(m, new object[] { 0 });

            Assert(m.NewsMemory.Count == 0, "Should be 0");
        }
        public void Test_Pruning_ExactlyAtLimit_NoOp() 
        {
            var c = CreateConfig(); 
            var m = new TACAMT_Model(c, new Random(42)); 
            ...
            typeof(TACAMT_Model).GetMethod("PruneNewsMemory", BindingFlags.Instance | BindingFlags.NonPublic).Invoke(m, new object[] { 5 });
            Assert(m.NewsMemory.Count == 5, "No-op");
        }
        public void Test_Pruning_Config_SavedAndLoaded() 
        {
            var m = new TACAMT_Model(CreateConfig(), new Random(42));
            m.PruningConfig.AttentionScoreAlpha = 0.2f;
            m.PruningConfig.MinQueryCountForPruning = 7;
            m.PruningConfig.NewEntryReserveFraction = 0.3f;
            m.PruningConfig.UseAttentionBasedPruning = false; 
            var d = GetTempDir(); 
            
            try 
            {
                m.Save(d); var ld = TACAMT_Model.Load(d); 
                Assert(MathF.Abs(ld.PruningConfig.AttentionScoreAlpha - 0.2f) < 1e-6f, "Alpha"); 
                Assert(ld.PruningConfig.MinQueryCountForPruning == 7, "MinQ");
                Assert(MathF.Abs(ld.PruningConfig.NewEntryReserveFraction - 0.3f) < 1e-6f, "Reserve"); 
                Assert(ld.PruningConfig.UseAttentionBasedPruning == false, "UseAttn"); 
            } 
            finally 
            { 
                CleanupDir(d); 
        ...
        #region Save/Load
        public void Test_SaveLoad_DecayParams_Preserved()
        {
            ...
            try
            {
                ...
                for (int h = 0; h < config.Price.NumHeads; h++)
                {
                    Assert(MathF.Abs(logDecayBefore[h] - loadedDecay.LogBaseDecayRate[h]) < 1e-6f, $"LogBaseDecayRate[{h}] mismatch after load");
                }

                Assert(!Array3DChanged(w1Before, loadedDecay.W1, 1e-6f), "W1 should be preserved after save/load");
            }
            finally { CleanupDir(dir); }
        }

        public void Test_SaveLoad_NewsMemory_Preserved()
        {
            ...
            try
            {
                ...
                Assert(loaded.NewsMemory.Count == memCount, $"Loaded memory count {loaded.NewsMemory.Count}, expected {memCount}");

                for (int i = 0; i < memCount; i++)
                {
                    Assert(Math.Abs(model.NewsMemory[i].AbsoluteTimestamp - loaded.NewsMemory[i].AbsoluteTimestamp) < 1e-6, "Memory timestamp mismatch after load");
                    for (int d = 0; d < config.Price.EmbeddingDim; d++)
                    {
                        Assert(MathF.Abs(model.NewsMemory[i].HiddenState[d] - loaded.NewsMemory[i].HiddenState[d]) < 1e-6f, "Memory hidden state mismatch");
                    }
                }
            }
            finally 
        ...
        public void Test_SaveLoad_ForwardMatch()
        {
            var (tokenizer, stories, priceInputs, _) = CreateTestData(numSamples: 1);
            var config = CreateConfig(tokenizer.VocabSize + 2);
            var model = new TACAMT_Model(config, new Random(42));
            ...
            try
            {
                ...
                for (int i = 0; i < pred1.GetLength(0); i++)
                {
                    for (int j = 0; j < pred1.GetLength(1); j++)
                    {
                        Assert(MathF.Abs(pred1[i, j] - pred2[i, j]) < 1e-5f, $"Forward mismatch at [{i},{j}] after save/load: {pred1[i, j]} vs {pred2[i, j]}");
                    }
                }
            }
            finally 
        ...
        public void Test_SaveLoad_ContinueTraining()
        {
            ...
            try
            {
                ...
                Assert(lossAfter <= lossMid * 1.1f, $"Training after load should not regress significantly: {lossMid:F6} -> {lossAfter:F6}");
            }
            finally 
            { 
                CleanupDir(dir);
        ...
        public void Test_SaveLoad_AllTextBlockWeights_Match() 
        { 
            var (tok, _, _, _) = CreateTestData(1);
            var c = CreateConfig(tok.VocabSize + 2, numLayers: 2); 
            var m = new TACAMT_Model(c, new Random(42)); 
            var d = GetTempDir();
            ...
            { 
                m.Save(d);
                var ld = TACAMT_Model.Load(d); 
                for (int i = 0; i < 2; i++) 
                {
                    Assert(!MatrixChanged(m.TextBlocks[i].Attention.WQ, ld.TextBlocks[i].Attention.WQ, 1e-6f), $"Block {i} WQ");
                    Assert(!MatrixChanged(m.TextBlocks[i].Attention.WK, ld.TextBlocks[i].Attention.WK, 1e-6f), $"Block {i} WK"); 
                } 
            } 
            finally 
            { 
        ...
        public void Test_SaveLoad_AllPriceBlockWeights_Match() 
        {
            var (tok, _, _, _) = CreateTestData(1);
            var c = CreateConfig(tok.VocabSize + 2, numLayers: 2);
            var m = new TACAMT_Model(c, new Random(42)); 
            ...
            { 
                ...
                for (int i = 0; i < 2; i++) 
                { 
                    Assert(!MatrixChanged(m.PriceBlocks[i].SelfAttention.WQ, ld.PriceBlocks[i].SelfAttention.WQ, 1e-6f), $"Block {i} SelfWQ");
                    Assert(!VectorChanged(m.PriceBlocks[i].LNSelfGamma, ld.PriceBlocks[i].LNSelfGamma, 1e-6f), $"Block {i} LN");
                } 
            }
            finally 
            { 
        ...
        public void Test_SaveLoad_PriceMemory_Preserved()
        {
            ...
            for (int i = 0; i < 3; i++) 
            { 
                var hs = new float[c.Price.EmbeddingDim];
                for (int dd = 0; dd < hs.Length; dd++) hs[dd] = (float)rng.NextDouble(); 
                m.PriceMemory.Add(new PriceMemoryEntry { HiddenState = hs, AbsoluteTimestamp = i * 100.0, AttentionScore = 0.5f, QueryCount = i });
            } 
            var d = GetTempDir();
            ...
            { 
                m.Save(d);
                var ld = TACAMT_Model.Load(d); 
                Assert(ld.PriceMemory.Count == 3, $"Got {ld.PriceMemory.Count}"); 
                for (int i = 0; i < 3; i++) 
                    Assert(Math.Abs(m.PriceMemory[i].AbsoluteTimestamp - ld.PriceMemory[i].AbsoluteTimestamp) < 1e-6, $"Ts {i}"); 
            } 
            finally 
            { 
                CleanupDir(d); 
        ...
        public void Test_SaveLoad_AttentionScores_Preserved() 
        {
            var c = CreateConfig();

            var m = new TACAMT_Model(c, new Random(42));

            m.NewsMemory.Add(new NewsMemoryEntry { HiddenState = new float[c.Price.EmbeddingDim], AbsoluteTimestamp = 1.0, AttentionScore = 0.42f, QueryCount = 7 }); 
            m.PriceMemory.Add(new PriceMemoryEntry { HiddenState = new float[c.Price.EmbeddingDim], AbsoluteTimestamp = 2.0, AttentionScore = 0.77f, QueryCount = 3 });
            ...
            try
            {
                m.Save(d);
                var ld = TACAMT_Model.Load(d);
                Assert(MathF.Abs(ld.NewsMemory[0].AttentionScore - 0.42f) < 1e-5f, "News AS"); 
                Assert(ld.NewsMemory[0].QueryCount == 7, "News QC");
                Assert(MathF.Abs(ld.PriceMemory[0].AttentionScore - 0.77f) < 1e-5f, "Price AS");
                Assert(ld.PriceMemory[0].QueryCount == 3, "Price QC"); 
            }
            finally
            {
                CleanupDir(d); 
        ...
        {
            var c = CreateConfig(textVocabSize: 77, embDim: 32, numHeads: 4, numLayers: 3, ffnDim: 64, inputFeatures: 7, outputDim: 3, priceSeqLen: 25, useConfidence: true, freezeText: true);
            var m = new TACAMT_Model(c, new Random(42)); var d = GetTempDir(); 
            
            try
            {
                m.Save(d);
                var ld = TACAMT_Model.Load(d);
                Assert(ld.Config.Text.VocabSize == 77, "Vocab");
                Assert(ld.Config.Text.EmbeddingDim == 32, "Emb");
                Assert(ld.Config.Text.NumHeads == 4, "Heads");
                Assert(ld.Config.Output.OutputDim == 3, "OD"); 
                Assert(ld.Config.Output.UseConfidenceHead == true, "Conf"); 
                Assert(ld.Config.Text.Freeze == true, "Freeze"); 
            }
            finally 
            { 
                CleanupDir(d);
        ...
        public void Test_Config_NegativeL2_Throws() 
        { 
            var c = CreateConfig(); 
            c.Regularization.L2RegulationLamda = -0.01f;
            bool t = false; 
            try 
            { 
                c.Validate(); 
        ...
        public void Test_Config_DecayDropoutOutOfRange_Throws()
        {
            var c = CreateConfig();
            c.DecayNetwork.MemAttentionDropout = 1.0f;
            bool t = false;
        ...
        public void Test_Config_ZeroDecayProjectionDim_Throws()
        { 
            var c = CreateConfig(); 
            c.DecayNetwork.ProjectionDim = 0;
            bool t = false;
        ...
        public void Test_Config_ZeroGradientClipThreshold_Throws()
        { 
            var c = CreateConfig();
            c.Regularization.GradientClippingThreshold = 0f;
            bool t = false;
        ...
        #region Decay network

        public void Test_Decay_LargerTimeDiff_StrongerDecay()
        {
            var rng = new Random(42);
            var accel = new AccelerationManagers.AccelerationCPU();
        ...
        public void Test_Decay_BaseDecayRate_AlwaysPositive() 
        { 
            var m = new TACAMT_Model(CreateConfig(numHeads: 4), new Random(42)); 
            for (int l = 0; l < m.Config.Price.NumLayers; l++)
            {
                for (int h = 0; h < 4; h++)
                {
                    Assert(MathF.Exp(m.PriceBlocks[l].DecayNetwork.LogBaseDecayRate[h]) > 0, $"L{l}H{h}");
                }
            }
        }
        public void Test_Decay_MultiScale_TimeEncoding_DifferentPerBase() 
        { 
            var net = new ContentAwareDecayNetwork(1, 16, 8, 16, new Random(42), numTimeBases: 4);
            bool d = false;

            for (int b = 1; b < 4 && !d; b++)
            {
                if (MathF.Abs(net.TimeLogFreq[0, 0] - net.TimeLogFreq[0, b]) > 1e-6f)
                {
                    d = true;
                }
            }
            Assert(d, "Should differ per base");
        }
        ...
        public void Test_Sequential_PriceMemoryGrows_EachSample() 
        {
            var (tok, st, pi, pt) = CreateTestData(3, priceSeqLen: 8);
            var c = CreateConfig(tok.VocabSize + 2, priceSeqLen: 8);

            var m = new TACAMT_Model(c, new Random(42));
            
            new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.001f, BatchSize = 1, Epochs = 1, Verbose = false }).TrainSequential(st, pi, pt, new double[] { 100, 200, 300 }, maxPriceMemory: 500);
           
            Assert(m.PriceMemory.Count > pi[0].GetLength(0) - 1, $"Got {m.PriceMemory.Count}"); 
        }
        public void Test_Sequential_MultiplEpochs_MemoryReset_EachEpoch()
        { 
            var (tok, st, pi, pt) = CreateTestData(2, priceSeqLen: 8);

            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2, priceSeqLen: 8), new Random(42)); m.NewsMemory.Add(new NewsMemoryEntry { HiddenState = new float[m.Config.Price.EmbeddingDim], AbsoluteTimestamp = 1.0 }); 
            new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.001f, BatchSize = 1, Epochs = 2, Verbose = false }).TrainSequential(st, pi, pt, new double[] { 100, 200 });
            
            Assert(m.NewsMemory.Count > 0, "Should have memory after");
        }
        public void Test_Sequential_WithNoStories_StillWorks()
        {
            var rng = new Random(42);
            int n = 3;
            var ns = new NewsStory[n][];
            ...
            for (int s = 0; s < n; s++)
            { 
                pi[s] = RandomMatrix(8, 5, rng, 0.5f);
                pt[s] = RandomMatrix(8, 5, rng, 0.5f);
            }

            var m = new TACAMT_Model(CreateConfig(priceSeqLen: 10), new Random(42)); 
            var t = new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.001f, BatchSize = 1, Epochs = 2, Verbose = false });

            t.TrainSequential(ns, pi, pt, new double[] { 100, 200, 300 }); Assert(!float.IsNaN(t.Validate(ns, pi, pt)), "Valid loss");
        }

        #endregion

        #region Price Context
        public void Test_PriceContext_SplitPointDistribution_WithinBounds() 
        { 
            int sl = 20, mh = 5, mc = 5; int maxH = sl - mc - 1; 
            var rng = new Random(42); 

            for (int trial = 0; trial < 100; trial++) 
            {
                int sp = mh + rng.Next(maxH - mh + 1);
                Assert(sp >= mh && sp <= maxH && sl - sp >= mc + 1, $"Split {sp} out of bounds");
            }
        }

        public void Test_PriceContext_HistoryDetached_CurrentTrained() 
        { 
            var (tok, st, pi, pt) = CreateTestData(5, priceSeqLen: 16);
            var m = new TACAMT_Model(CreateConfig(tok.VocabSize + 2, priceSeqLen: 16), new Random(42));
            var ob = CloneMatrix(m.OutputProjection);
            new TACAMT_Trainer(m, new TrainingConfig { LearningRate = 0.01f, BatchSize = 5, Epochs = 5, Verbose = false }).Train(st, pi, pt);

            Assert(MatrixChanged(ob, m.OutputProjection), "OutputProjection should change from current chunk");
        }
        #endregion

        #region Text encoder

        public void Test_TextEncoder_Bidirectional_Default()
        { 
            Assert(CreateConfig().Text.UseDecoderOnly == false, "Should be bidirectional"); 
        }
        public void Test_TextEncoder_MeanPooling_Produces_FixedDimOutput() 
        { 
            var (tok, stories, pi, _) = CreateTestData(1);
            var c = CreateConfig(tok.VocabSize + 2, embDim: 16);
            var m = new TACAMT_Model(c, new Random(42));
            ...
            Assert(cache.TextFinalHidden.GetLength(0) == stories[0].Length, $"Stories {cache.TextFinalHidden.GetLength(0)} vs {stories[0].Length}");
            Assert(cache.TextFinalHidden.GetLength(1) == 16, $"Dim {cache.TextFinalHidden.GetLength(1)}");
        }
        ...
        public void Test_Tokenizer_MatchingVocab_AcceptedAndUsable() 
        { 
            var tok = new BPETokenizer();
            tok.Train(new[] { "stock price rose" }, vocabSize: 50, minFrequency: 1);
            var m = new TACAMT_Model(CreateConfig(textVocabSize: tok.VocabSize), new Random(42)); m.SetTokenizer(tok);
            var s = m.TokenizeStories(new[] { "stock price" }, new[] { 0f }); Assert(s.Length == 1 && s[0].TokenIds.Length > 0, "Failed");
        }
        public void Test_Tokenizer_SavedWithModel_LoadedBack() 
        {
            var tok = new BPETokenizer();

            tok.Train(new[] { "stock price rose sharply" }, vocabSize: 50, minFrequency: 1);
            ...
            { 
                m.Save(d);
                var ld = TACAMT_Model.Load(d); 
                Assert(ld.Tokenizer != null, "Should load");
                Assert(ld.Tokenizer.Encode("stock").Length > 0, "Should encode"); 
            }
            finally 
            { 
                CleanupDir(d);
        ...
        public void Test_Stability_VeryLargeInputs_NoNaN() 
        {
            var m = new TACAMT_Model(CreateConfig(priceSeqLen: 12), new Random(42));
            var p = new float[10, 5];
            ...
            Assert(!HasNaN(m.Forward((NewsStory[])null, p).predictions), "NaN");
        }
        public void Test_Stability_VerySmallInputs_NoNaN() 
        {
            var m = new TACAMT_Model(CreateConfig(priceSeqLen: 12), new Random(42));
            var p = new float[10, 5];
        ...
        public void Test_Stability_ZeroInputs_NoNaN() 
        { 
            var m = new TACAMT_Model(CreateConfig(priceSeqLen: 12), new Random(42)); 

            Assert(!HasNaN(m.Forward((NewsStory[])null, new float[10, 5]).predictions), "NaN");
        }
        public void Test_Stability_NegativeInputs_NoNaN()
        {
            var m = new TACAMT_Model(CreateConfig(priceSeqLen: 12), new Random(42));
            var p = new float[10, 5];
        ...
        {
            var m = new TACAMT_Model(CreateConfig(priceSeqLen: 12), new Random(42));
            var rng = new Random(42); var p = new float[10, 5];

            for (int i = 0; i < 10; i++)
            {
                for (int j = 0; j < 5; j++)
                {
                    p[i, j] = (float)Math.Pow(10, rng.Next(-3, 3)) * (rng.NextSingle() > 0.5f ? 1f : -1f);
                }
            }
        ...
        #region Learning Quality - synthetic groud truth
        ...
        public void Test_Decay_ContentAware_HighImpactStory_HasLargerOutput_AtSameAge()
        {
            var tok = new BPETokenizer();
            tok.Train(
                new[]
                {
            "emergency central bank intervention market shock",
            "minor commentary routine market update"
                },
                vocabSize: 80,
                minFrequency: 1);

            var highImpactTokens = tok.Encode(
                "emergency central bank intervention market shock",
                addSpecialTokens: true);

            var lowImpactTokens = tok.Encode(
                "minor commentary routine market update",
                addSpecialTokens: true);

            // Keep sequence length below default split threshold:
            // MinHistoryLength(5) + MinCurrentLength(5) + 1 = 11
            const int trainingSeqLen = 8;
            const int probeSeqLen = 7;
            ...
            for (int s = 0; s < numSamples; s++)
            {
                ...
                stories[s] = new[]
                {
            new NewsStory(isHigh ? highImpactTokens : lowImpactTokens, age)
        };

                priceInputs[s] = ZeroMatrix(trainingSeqLen, 3);
                priceTargets[s] = new float[trainingSeqLen, 1];

                float target = isHigh ? 1.0f : 0.2f;

                for (int t = 0; t < trainingSeqLen; t++)
                {
                    priceTargets[s][t, 0] = target;
                }
            }
            ...
            var probePrice = ZeroMatrix(probeSeqLen, 3);

            var (predHigh, _) = model.Forward(
                new[] { new NewsStory(highImpactTokens, age) },
                probePrice);

            var (predLow, _) = model.Forward(
                new[] { new NewsStory(lowImpactTokens, age) },
                probePrice);

            float highVal = predHigh[probeSeqLen - 1, 0];
            float lowVal = predLow[probeSeqLen - 1, 0];

            Assert(highVal > lowVal + 0.05f,
                $"Expected equally-old high-impact story to produce larger output than equally-old low-impact story. " +
                $"High={highVal:F6}, Low={lowVal:F6}");
        }
        public void Test_E2E_ContentAwareDecay_WithCompetition()
        {
            var tok = new BPETokenizer();
            tok.Train(
                new[]
            ...
            for (int s = 0; s < numSamples; s++)
            {
                ...
                bool highIsRecent = (s % 2 == 0);

                var highStory = new NewsStory(
                    highImpactTokens,
                    highIsRecent ? recentTime : oldTime);

                var lowStory = new NewsStory(
                    lowImpactTokens,
                    highIsRecent ? oldTime : recentTime);

                stories[s] = new[] { highStory, lowStory };

                float decayHigh = MathF.Exp(highStory.ArrivalTime / 10f);
                float decayLow = MathF.Exp(lowStory.ArrivalTime / 10f);

                float contribHigh = decayHigh * 1.0f;
                float contribLow = decayLow * 0.2f;

                float target = MathF.Max(contribHigh, contribLow);
            ...
            var (pred1, _) = model.Forward(
                new[]
                {
            new NewsStory(highImpactTokens, recentTime),
            new NewsStory(lowImpactTokens, oldTime)
                },
                probePrice);

            var (pred2, _) = model.Forward(
                new[]
                {
            new NewsStory(highImpactTokens, oldTime),
            new NewsStory(lowImpactTokens, recentTime)
                },
                probePrice);

            float case1 = pred1[probeSeqLen - 1, 0];
            float case2 = pred2[probeSeqLen - 1, 0];

            Assert(case1 > case2 + 0.05f,
                $"Recent high-impact should dominate high old + low recent. " +
                $"Case1={case1:F6}, Case2={case2:F6}");
        }
        public void Test_Decay_ContentAware_OlderHighImpact_Can_Outweigh_NewerLowImpact()
        {
            var tok = new BPETokenizer();
            tok.Train(
                new[]
            ...
            float oldHighAge = -8f;
            float recentLowAge = -1f;

            for (int s = 0; s < numSamples; s++)
            {
                bool oldHighShouldWin = (s % 2 == 0);

                stories[s] = oldHighShouldWin
                    ? new[]
                    {
                new NewsStory(highImpactTokens, oldHighAge),
                new NewsStory(lowImpactTokens, recentLowAge)
                    }
                    : new[]
                    {
                new NewsStory(lowImpactTokens, oldHighAge),
                new NewsStory(highImpactTokens, recentLowAge)
                    };

                priceInputs[s] = ZeroMatrix(trainingSeqLen, 3);
                priceTargets[s] = new float[trainingSeqLen, 1];

                float target = oldHighShouldWin ? 0.70f : 0.30f;
            ...
            var (predOldHighWins, _) = model.Forward(
                new[]
                {
            new NewsStory(highImpactTokens, oldHighAge),
            new NewsStory(lowImpactTokens, recentLowAge)
            ...
            var (predRecentHighWins, _) = model.Forward(
                new[]
                {
            new NewsStory(lowImpactTokens, oldHighAge),
            new NewsStory(highImpactTokens, recentLowAge)
            ...
            float oldHighCase = predOldHighWins[probeSeqLen - 1, 0];
            float recentHighCase = predRecentHighWins[probeSeqLen - 1, 0];

            Assert(oldHighCase > recentHighCase + 0.05f,
                $"Older high-impact story should be able to outweigh newer low-impact story in the supervised setup. " +
                $"OldHighCase={oldHighCase:F6}, RecentHighCase={recentHighCase:F6}");
        }
        public void Test_Decay_ContentAware_RecentSameContentStory_Beats_OlderSameContentStory_UnderCompetition()
        {
            var tok = new BPETokenizer();
            tok.Train(
                new[]
            ...
            var targetTokens = tok.Encode("emergency central bank intervention market shock", addSpecialTokens: true);

            var distractorTokens = tok.Encode("minor commentary routine market update", addSpecialTokens: true);
            ...
            float recentTime = -1f;
            float oldTime = -12f;
            float distractorTime = -4f;

            for (int s = 0; s < numSamples; s++)
            {
                bool targetIsRecent = (s % 2 == 0);

                var targetStory = new NewsStory(targetTokens, targetIsRecent ? recentTime : oldTime);

                var distractorStory = new NewsStory(distractorTokens, distractorTime);

                stories[s] = new[] { targetStory, distractorStory };

                priceInputs[s] = ZeroMatrix(trainingSeqLen, 3);
                priceTargets[s] = new float[trainingSeqLen, 1];

                float target = targetIsRecent ? 0.90f : 0.45f;
            ...
            var (predRecent, _) = model.Forward(
                new[]
                {
                    new NewsStory(targetTokens, recentTime),
                    new NewsStory(distractorTokens, distractorTime)
                },
                probePrice);

            var (predOld, _) = model.Forward(
                new[]
                {
                    new NewsStory(targetTokens, oldTime),
                    new NewsStory(distractorTokens, distractorTime)
                },
                probePrice);

            float recentVal = predRecent[probeSeqLen - 1, 0];
            float oldVal = predOld[probeSeqLen - 1, 0];

            Assert(recentVal > oldVal + 0.05f, $"Expected recent same-content story to beat older same-content story under competition. Recent={recentVal:F6}, Old={oldVal:F6}");
        }

        public void Test_TimeDecay_InitialValues()
        {
            var config = CreateConfig();
            var model = new TACAMT_Model(config, new Random(42));
            var decayNet = model.PriceBlocks[0].DecayNetwork;
        ...
        public void Test_TimeDecay_ParamsChangeAfterTraining()
        {
            var (tokenizer, stories, priceInputs, priceTargets) = CreateTestData(numSamples: 5);
            var config = CreateConfig(tokenizer.VocabSize + 2);
            var model = new TACAMT_Model(config, new Random(42));
            ...
            Assert(VectorChanged(logDecayBefore, decayNet.LogBaseDecayRate), "LogBaseDecayRate should change after training");

            Assert(Array3DChanged(w1Before, decayNet.W1), "W1 should change after training");
        }

        public void Test_BackwardCompat_NullText_NoError()
        {
            var config = CreateConfig();
            ...
            for (int i = 0; i < 10; i++)
            {
                for (int j = 0; j < 5; j++)
                {
                    price[i, j] = (float)rng.NextDouble();
                }
            }
        ...
        public void Test_BackwardCompat_TrainWithIntArrays()
        {
        ...
        public void Test_MixedBatch_SomeStoriesSomeNull()
        {
        ...
        public void Test_NegativeArrivalTime_NoError()
        {
            ...
            var stories = new[]
            {
                new NewsStory(tokens, -5f),
                new NewsStory(tokens, -2f)
            };
        ...
        public void Test_ManyStories_NoError()
        {
            var (tokenizer, _, priceInputs, _) = CreateTestData(numSamples: 1);
        ...
        public void Test_SingleTokenStory_NoError()
        {
            var config = CreateConfig();
            var model = new TACAMT_Model(config, new Random(42));
            var rng = new Random(42);
            ...
            for (int i = 0; i < 10; i++)
            {
                for (int j = 0; j < 5; j++) price[i, j] = (float)rng.NextDouble();
            }

            var stories = new[]
            {
                new NewsStory(new[] { 1 }, 0f)
            };
        ...
        public void Test_UpdateNewsMemory_StoresEntries()
        {
            var (tokenizer, stories, _, _) = CreateTestData(numSamples: 1);
            ...
            Assert(model.NewsMemory.Count == stories[0].Length, $"Memory count: {model.NewsMemory.Count}, expected {stories[0].Length}");
        }

        public void Test_ClearNewsMemory_Resets()
        {
            var (tokenizer, stories, _, _) = CreateTestData(numSamples: 1);
        ...
        public void Test_UpdateNewsMemory_TrimOldEntries()
        {
            var (tokenizer, stories, _, _) = CreateTestData(numSamples: 1);
            var config = CreateConfig(tokenizer.VocabSize + 2);
            var model = new TACAMT_Model(config, new Random(42));
        ...
        public void Test_PredictWithMemory_NoMemory_FallsBack()
        {
            var (tokenizer, stories, priceInputs, _) = CreateTestData(numSamples: 1);
        ...
        public void Test_PredictWithMemory_WithMemory_NoError()
        {
        ...
        public void Test_PredictWithMemory_OldStoriesDecayed()
        {
            var (tokenizer, stories, priceInputs, _) = CreateTestData(numSamples: 1);
            var config = CreateConfig(tokenizer.VocabSize + 2);
            var model = new TACAMT_Model(config, new Random(42));
            ...
            {
                if (MathF.Abs(pred1[j] - pred2[j]) > 1e-6f)
                {
                    anyDiff = true;
                }
            }
        ...
        public void Test_Deterministic_MultiStory()
        {
            var (tokenizer, stories, priceInputs, _) = CreateTestData(numSamples: 1);
            var config = CreateConfig(tokenizer.VocabSize + 2);
            var model = new TACAMT_Model(config, new Random(42));
        ...
        public void Test_GradientClipping_WithDecay_NoNaN()
        {
            var (tokenizer, stories, priceInputs, priceTargets) = CreateTestData(numSamples: 5);
            var config = CreateConfig(tokenizer.VocabSize + 2);
            var model = new TACAMT_Model(config, new Random(42));
        ...
        public void Test_Validate_MultiStory()
        {
            var (tokenizer, stories, priceInputs, priceTargets) = CreateTestData(numSamples: 5);
            var config = CreateConfig(tokenizer.VocabSize + 2);
            var model = new TACAMT_Model(config, new Random(42));
        ...
        public void Test_PerHeadDecayRates_Diverge()
        {
            var (tokenizer, stories, priceInputs, priceTargets) = CreateTestData(numSamples: 10);
        ...
        public void Test_PriceOnly_ForwardNoError()
        {
            var config = CreateConfig();
            var model = new TACAMT_Model(config, new Random(42));
            var rng = new Random(42);
        ...
        public void Test_PriceOnly_LossDecreases()
        {
            var rng = new Random(42);
            int numSamples = 5;
            var nullStories = new NewsStory[numSamples][];
        ...
        public void Test_FrozenTextEncoder_DecayStillLearns()
        {
            var (tokenizer, stories, priceInputs, priceTargets) = CreateTestData(numSamples: 5);
            ...
            Assert(!MatrixChanged(textEmbBefore, model.TextTokenEmbedding), "Text embedding should NOT change when frozen");
            Assert(VectorChanged(logDecayBefore, decayNet.LogBaseDecayRate), "Decay LogBaseDecayRate should still change when text is frozen");
        }
        ...
        public void Test_MemoryPruning_AttentionBased_KeepsHighScoreEntries()
        {
            ...
            var method = typeof(TACAMT_Model).GetMethod("PruneNewsMemory", BindingFlags.Instance | BindingFlags.NonPublic);
            ...
            Assert(minScore >= 0.4f, $"Lowest surviving score should be high (≥0.4), got {minScore}");
        }
        ...
        public void Test_DecayNetwork_ForwardProduces_ValidBias()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 16; j++)
                {
                    queryEmb[i, j] = (float)rng.NextDouble() - 0.5f;
                }
            }
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 16; j++)
                {
                    keyEmb[i, j] = (float)rng.NextDouble() - 0.5f;
                }
            }
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    timeDiffs[i, j] = MathF.Abs(i - keyTimes[j]);
                }
            }
        ...
        public void Test_DecayNetwork_BackwardProduces_NonZeroGrads()
        {
            ...
            {
                if (MathF.Abs(grads.LogBaseDecayRateGrad[h]) > 1e-12f)
                {
                    anyNonZero = true;
                }
            }
            ...
            for (int i = 0; i < 4 && !queryGradNonZero; i++)
            {
                ...
                {
                    if (MathF.Abs(dQuery[i, j]) > 1e-12f)
                    {
                        queryGradNonZero = true;
                    }
                }
            }
        ...
        public void Test_DecayNetwork_WeightDecay_AddedToGrads()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 16; j++)
                {
                    queryEmb[i, j] = (float)rng.NextDouble();
                }
            }
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 16; j++)
                {
                    keyEmb[i, j] = (float)rng.NextDouble();
                }
            }
            for (int i = 0; i < 4; i++)
            {
        ...
        public void Test_DecayNetwork_SaveLoad_Roundtrip()
        {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                Assert(MathF.Abs(loaded.TimeNormalizationHours - net.TimeNormalizationHours) < 1e-6f, "TimeNormalizationHours mismatch");
                Assert(MathF.Abs(loaded.InitialHalfLifeHours - net.InitialHalfLifeHours) < 1e-6f, "InitialHalfLifeHours mismatch");
                Assert(MathF.Abs(loaded.MinHalfLifeHours - net.MinHalfLifeHours) < 1e-6f, "MinHalfLifeHours mismatch");
                Assert(MathF.Abs(loaded.MaxHalfLifeHours - net.MaxHalfLifeHours) < 1e-6f, "MaxHalfLifeHours mismatch");
                Assert(MathF.Abs(loaded.MinGate - net.MinGate) < 1e-6f, "MinGate mismatch");
                Assert(MathF.Abs(loaded.MaxGate - net.MaxGate) < 1e-6f, "MaxGate mismatch");
        ...
        public void Test_ConfidenceHead_Outputs_SigmoidRange()
        {
            ...
            for (int i = 0; i < conf.GetLength(0); i++)
            {
                Assert(conf[i, 0] >= 0f && conf[i, 0] <= 1f, $"Confidence[{i}] = {conf[i, 0]} outside [0,1]");
            }
        }
        ...
        public void Test_Tokenizer_TokenizeStories_WithoutTokenizer_Throws()
        {
            ...
            try
            {
                model.TokenizeStories(new[] { "hello" }, new[] { 0f });
            }
        ...
        public void Test_Dims_TextEmbedding_MatchesConfig()
        {
            var c = CreateConfig(textVocabSize: 100, embDim: 32);

            var m = new TACAMT_Model(c, new Random(42));

            Assert(m.TextTokenEmbedding.GetLength(0) == 100, $"Rows {m.TextTokenEmbedding.GetLength(0)}");

            Assert(m.TextTokenEmbedding.GetLength(1) == 32, $"Cols {m.TextTokenEmbedding.GetLength(1)}");
        }
        public void Test_Dims_PriceInputProjection_MatchesConfig()
        {
            var c = CreateConfig(embDim: 32, inputFeatures: 7);
            var m = new TACAMT_Model(c, new Random(42));
            Assert(m.PriceInputProjection.GetLength(0) == 32, "Rows");
            Assert(m.PriceInputProjection.GetLength(1) == 7, "Cols");
        }
        public void Test_Dims_OutputProjection_MatchesConfig()
        {
            var c = CreateConfig(embDim: 16, outputDim: 3);
            var m = new TACAMT_Model(c, new Random(42));
            Assert(m.OutputProjection.GetLength(0) == 3, "Rows");
            Assert(m.OutputProjection.GetLength(1) == 16, "Cols");
        }
        public void Test_Dims_AllBlocks_SelfAttn_WQ_Correct()
        {
            var c = CreateConfig(embDim: 16, numLayers: 3);
            var m = new TACAMT_Model(c, new Random(42));
            for (int i = 0; i < 3; i++)
            {
                Assert(m.PriceBlocks[i].SelfAttention.WQ.GetLength(0) == 16, $"Block {i} WQ r");
                Assert(m.PriceBlocks[i].SelfAttention.WQ.GetLength(1) == 16, $"Block {i} WQ c");
            }
        }
        public void Test_Dims_AllBlocks_CrossAttn_WQ_Correct()
        {
            var c = CreateConfig(embDim: 16, numLayers: 2);
            var m = new TACAMT_Model(c, new Random(42));
            for (int i = 0; i < 2; i++)
            {
                Assert(m.PriceBlocks[i].CrossAttention.WQ.GetLength(0) == 16, $"Block {i}");
                Assert(m.PriceBlocks[i].CrossAttention.WQ.GetLength(1) == 16, $"Block {i}");
            }
        }
        public void Test_Dims_DecayNetwork_Projections_Correct()
        {
            var c = CreateConfig(numHeads: 4, embDim: 16);
            var m = new TACAMT_Model(c, new Random(42));
            var dn = m.PriceBlocks[0].DecayNetwork;
            Assert(dn.QueryProjection.GetLength(0) == 4, "heads");
            Assert(dn.QueryProjection.GetLength(1) == c.DecayNetwork.ProjectionDim, "projDim");
            Assert(dn.QueryProjection.GetLength(2) == 16, "contentDim");
            Assert(dn.W1.GetLength(2) == c.DecayNetwork.ProjectionDim * 3 + 2, "MLPInputDim");
        }
        ...
        public void Test_Dims_LayerNormVectors_CorrectLength()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                Assert(m.PriceBlocks[i].LNSelfGamma.Length == 16, $"Block {i}");
                Assert(m.PriceBlocks[i].LnCrossGamma.Length == 16, $"Block {i}");
                Assert(m.PriceBlocks[i].LNFFNGamma.Length == 16, $"Block {i}");
            }
        }
        public void Test_Dims_ConfidenceProjection_When_Disabled_IsNull()
        {
            var m = new TACAMT_Model(CreateConfig(useConfidence: false), new Random(42));
            Assert(m.ConfidenceProjection == null, "Should be null");
        }
        public void Test_Dims_ConfidenceProjection_When_Enabled_Correct()
        {
            var m = new TACAMT_Model(CreateConfig(useConfidence: true, embDim: 16), new Random(42)); Assert(m.ConfidenceProjection != null, "Not null");
        ...
        public void Test_ContextTypeEmbedding_SaveLoadRoundTrip()
        {
            ...
            try
            {
                ...
                for (int t = 0; t < 2; t++)
                {
                    for (int d = 0; d < c.Price.EmbeddingDim; d++)
                    {
                        Assert(model.ContextTypeEmbedding[t, d] == loaded.ContextTypeEmbedding[t, d], $"Type embedding [{t},{d}] mismatch after load: {model.ContextTypeEmbedding[t, d]} vs {loaded.ContextTypeEmbedding[t, d]}");
                    }
                }
            }
            finally
        ...
        public void Test_ContextTypeEmbedding_IncludedInGradientClipping()
        {
            ...
            for (int t = 0; t < 2; t++)
            {
                for (int d = 0; d < c.Price.EmbeddingDim; d++)
                {
                    Assert(!float.IsNaN(model.ContextTypeEmbedding[t, d]) && !float.IsInfinity(model.ContextTypeEmbedding[t, d]), $"ContextTypeEmbedding[{t},{d}] is NaN/Inf - gradient clipping may not include it");
                }
            }
            ...
            Assert(maxChange < 5.0f, $"Max change {maxChange:F4} seems too large - gradient clipping may not be applied to type embeddings");
        }
        ...
        {
            // Directly verify the thing decay is supposed to change:
            // attention weight on the recent key should exceed attention weight
            // on the old key when content is identical and only age differs.
            //
            // We use two identical stories so K/V content is the same and the only
            // difference is the arrival time / age seen by the decay mechanism.
            // Then we inspect the cached cross-attention weights instead of using
            // end-to-end output movement as a proxy.

            var config = CreateConfig(priceSeqLen: 8);
            var model = new TACAMT_Model(config, new Random(42));

            // Constant price input removes price variation as a confounder.
            ...
            // Two identical stories, only age differs.
            // Entry 0 = recent (age 1)
            // Entry 1 = old    (age 1000)
            var tokens = new[] { 4, 5, 6, 7 };
            var stories = new[]
            {
        ...
        new NewsStory(tokens, -1000f)
            ...
            float weightRecent = 0f;
            float weightOld = 0f;
            int contributingLayers = 0;

            for (int layer = 0; layer < cache.PriceBlockCaches.Count; layer++)
            {
                var bc = cache.PriceBlockCaches[layer];

                Assert(bc.CrossAttentionWeights != null, $"Cross attention weights were not cached for price block {layer}");

                int numHeads = bc.CrossAttentionWeights.Length;
                Assert(numHeads > 0, $"No cross-attention heads cached for price block {layer}");

                for (int h = 0; h < numHeads; h++)
                {
                    var w = bc.CrossAttentionWeights[h];
                    Assert(w != null, $"CrossAttentionWeights[{h}] is null for price block {layer}");
                    Assert(w.GetLength(1) >= 2, $"Expected at least 2 context keys in cached attention for price block {layer}, got {w.GetLength(1)}");

                    // w[queryPos, keyPos]
                    for (int q = 0; q < w.GetLength(0); q++)
                    {
                        weightRecent += w[q, 0]; // story 0 = recent
                        weightOld += w[q, 1];    // story 1 = old
                    }
                }
            ...
            Assert(contributingLayers > 0, "No price blocks contributed cached cross-attention weights");

            Assert(weightRecent > weightOld, $"Recent memory/story should receive more total cross-attention weight than old when content is identical. Got recent={weightRecent:F6}, old={weightOld:F6}");
        }
        public void Test_Decay_ContentAware_SameAgeDifferentContent_ChangesPrediction()
        {
            var tok = new BPETokenizer();
            tok.Train(new[]
            {
        ...
        "minor commentary routine market update"
            ...
            var highImpactTokens = tok.Encode("emergency central bank intervention market shock", addSpecialTokens: true);
            var lowImpactTokens = tok.Encode("minor commentary routine market update", addSpecialTokens: true);
            ...
            for (int s = 0; s < numSamples; s++)
            {
                priceInputs[s] = RandomMatrix(12, 3, rng, 0.2f);
                priceTargets[s] = new float[12, 1];

                bool high = (s % 2 == 0);
                stories[s] = new[]
                {
            new NewsStory(high ? highImpactTokens : lowImpactTokens, -20f)
        };

                FillTarget(priceTargets[s], high ? 1.0f : 0.0f);
            }
            ...
            var probePrice = RandomMatrix(12, 3, new Random(99), 0.2f);

            var (predHigh, _) = model.Forward(new[] { new NewsStory(highImpactTokens, -20f) }, probePrice);
            var (predLow, _) = model.Forward(new[] { new NewsStory(lowImpactTokens, -20f) }, probePrice);

            float highVal = predHigh[predHigh.GetLength(0) - 1, 0];
            float lowVal = predLow[predLow.GetLength(0) - 1, 0];

            Assert(MathF.Abs(highVal - lowVal) > 0.15f,
                $"Same-age different-content stories should produce meaningfully different predictions. " +
                $"High={highVal:F6}, Low={lowVal:F6}");
        }
        ...
        private (BPETokenizer tokenizer, NewsStory[][] stories, float[][,] priceInputs, float[][,] priceTargets) CreateTestData(int numSamples = 10, int priceSeqLen = 16, int inputFeatures = 5, int outputDim = 5, int seed = 42)
        {
            ...
            string[] corpus = { "stock price rose sharply today", "market crashed due to earnings miss", "bullish sentiment on tech sector" };
            ...
            for (int s = 0; s < numSamples; s++)
            {
                int ns2 = 2 + random.Next(2); stories[s] = new NewsStory[ns2];

                for (int ns = 0; ns < ns2; ns++)
                {
                    var tokens = tokenizer.Encode(corpus[random.Next(corpus.Length)], addSpecialTokens: true);
                    stories[s][ns] = new NewsStory(tokens, ns * 3.0f);
                }

                priceInputs[s] = new float[priceSeqLen, inputFeatures];
                priceTargets[s] = new float[priceSeqLen, outputDim];
                float bp = 100f + (float)(random.NextDouble() * 50);

                for (int t = 0; t < priceSeqLen; t++)
                {
                    for (int f = 0; f < inputFeatures; f++)
                    {
                        priceInputs[s][t, f] = (bp + (float)(random.NextDouble() - 0.5) * 10f) / 200f;
                    }
                    for (int f = 0; f < outputDim; f++)
                    {
                        priceTargets[s][t, f] = (bp + (float)(random.NextDouble() - 0.5) * 10f) / 200f; bp += (float)(random.NextDouble() - 0.48) * 2f;
                    }
                }
            }
        ...
        private MultimodalTransformerConfig CreateConfig(int textVocabSize = 50, int embDim = 16, int numHeads = 2, int numLayers = 1, int ffnDim = 32, int inputFeatures = 5, int outputDim = 5, int priceSeqLen = 20, bool useConfidence = false, bool freezeText = false, int priceCtxMinHist = 5, int priceCtxMinCurrent = 5)
        {
            var config = new MultimodalTransformerConfig
            {
                Text = new TextEncoderConfig
        ...
        private bool Array3DChanged(float[,,] before, float[,,] after, float threshold = 1e-10f)
        {
            for (int i = 0; i < before.GetLength(0); i++)
            {
                for (int j = 0; j < before.GetLength(1); j++)
                {
                    ...
                    {
                        if (MathF.Abs(before[i, j, k] - after[i, j, k]) > threshold)
                        {
                            return true;
                        }
                    }
        ...
        private void FillTarget(float[,] target, float value)
        {
            for (int t = 0; t < target.GetLength(0); t++)
            {
                target[t, 0] = value;
            }
        }
        ...
        {
            return (float[,])m.Clone();
        }
        private float[] CloneVector(float[] v)
        {
            return (float[])v.Clone();
        }
        private float[,,] Clone3D(float[,,] a)
        {
        ...
        private bool HasNaN(float[,] m)
        {
            for (int i = 0; i < m.GetLength(0); i++)
            {
                ...
                {
                    if (float.IsNaN(m[i, j]) || float.IsInfinity(m[i, j]))
                    {
                        return true;
                    }
                }
        ...
        private float[,] RandomMatrix(int rows, int cols, Random rng, float scale = 1f)
        {
